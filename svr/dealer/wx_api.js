...
const wxpay = new WXPay({
    appId: credential.wx_APPID,
    mchId: credential.wx_MCHID,
    key: credential.wx_KEY,
    certFileContent: fs.readFileSync('./keys/apiclient_cert.p12'),
    caFileContent: fs.readFileSync('./keys/rootca.pem')
});
class WxApi {
    constructor() {
        _.bindAll(this, ['init_rest', 'init_sio', 'init_ws', 'req_wxpay_qr']);
    ...
    init_rest(app) {
        app.post('/wx_notify', (req, res) => {
            // console.log("wx qr pay callback...", req.body);
            const resp = req.body.xml;
            logger.info("wx qr pay callback...", resp);
            if ( !wxpay.isPayResultNotifySignatureValid(resp) ) {
                logger.error("invalid wx notified data, skip...");
            ...
            if (resp.return_code == 'SUCCESS' && resp.result_code == 'SUCCESS') {
                ...
                // console.log('notify pay failed', resp.result_code);
                logger.error('notify pay failed', resp.result_code);               
            }
            res.end('success');
        });
    }
    init_sio(socket) {
        socket.on('req_wxpay_qr', (data, cb) => this.req_wxpay_qr(socket, data, cb));
    }
    init_ws(ws, req) {
    ...
    async req_wxpay_qr(sock, data, cb) {
        try {
            // console.log('req_wxpay_qr', data)
            const mch = await util.verify_req(data, () => data.cli_id);
            const reqObj = this.get_req_obj(sock, data, mch);
            reqObj.trade_type = 'NATIVE';
            const res = await wxpay.unifiedOrder(reqObj);
            if(res.return_code !== 'SUCCESS') throw res.return_msg;
...
//             logger.error('notify pay failed', resp.result_code[0]);
//             res.end('success');
//         }
//     });
